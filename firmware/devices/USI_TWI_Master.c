...
unsigned char USI_TWI_Start_Transceiver_With_Data(unsigned char *msg, unsigned char msgSize)
{
  ...
  USI_TWI_state.state.addressMode = TRUE;     // Always true for first byte
  ...
  if (!(*msg & (1<<TWI_READ_BIT)))                // The LSB in the address byte determines if is a masterRead or masterWrite operation.
    {
      USI_TWI_state.state.masterWriteDataMode = TRUE;
    }

  //  if (USI_TWI_state.state.memReadMode)
  //  {
  savedMsg = msg;
  ...
  do
    {
      ...
      if (USI_TWI_state.state.addressMode || USI_TWI_state.state.masterWriteDataMode)
        {
          /* Write a byte */
          ...
          if ((!USI_TWI_state.state.addressMode) && USI_TWI_state.state.memReadMode)// means memory start address has been written
            {
            ...
            {
              USI_TWI_state.state.addressMode = FALSE;            // Only perform address transmission once.
            }
        }
